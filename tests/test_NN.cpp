...
std::vector<float> loadCSV(const std::string& filename, int& numDays, int& numFeatures)
{
    ...
    while (std::getline(file, line))
    {
        ...
        std::getline(ss, value, ',');
        float close = std::stof(value);
        std::getline(ss, value, ',');
        float volume = std::stof(value);
        std::getline(ss, value, ',');
        float open = std::stof(value);
        std::getline(ss, value, ',');
        float high = std::stof(value);
        std::getline(ss, value, ',');
        float low = std::stof(value);
...
void testNeuralNetInitialization()
{
    ...
    std::vector<float> trainingData = loadCSV(
        "C:/Users/Michael/Downloads/HistoricalData_1738438947373.csv", numDays, numFeatures);

    // Print some debug info
    std::cout << "Loaded " << numDays << " days of data with " << numFeatures << " features each."
              << std::endl;
