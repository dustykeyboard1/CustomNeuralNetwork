...
__global__ void crossEntropyLossKernel(const float* yTrue, const float* yPred, float* loss,
                                       int numClasses, int batchSize)
{
    extern __shared__ float sharedLoss[];
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
float LossOps::MeanSquaredError(const float* targets, const float* predictions, int size,
                                bool isGPU)
{
...
float LossOps::gpuCrossEntropyLoss(const float* yTrue, const float* yPred, int batchSize,
                                   int numClasses)
{
    float* d_yTrue;
    float* d_yPred;
...
void LossOps::computeError(const float* yTrue, const float* yPred, float* error, int size,
                           bool isGPU)
{
