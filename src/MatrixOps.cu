...
__global__ void MultiplicationKernel(const float* A, const float* B, float* C, int rowsA, int colsA,
                                     int rowsB, int colsB)
{
    int row = blockIdx.y * blockDim.y + threadIdx.y;
    int col = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void initializeWeightsKernel(float* weights, int rows, int cols, curandState* states,
                                        int initTypeCode)
{
    int row = blockIdx.y * blockDim.y + threadIdx.y;
    int col = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void AddBiasKernel(const float* output, const float* bias, float* result, int batchSize,
                              int outputSize)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void elementWiseMultiplyKernel(const float* A, const float* B, float* C, int rows,
                                          int cols)
{
    int row = blockIdx.y * blockDim.y + threadIdx.y;
    int col = blockIdx.x * blockDim.x + threadIdx.x;
              ...
              bool isGPU)
{
    ...
    if (isGPU)
    {
        MultiplicationKernel<<<blocksPerGrid, threadsPerBlock>>>(A, B, C, rowsA, colsA, rowsB,
                                                                 colsB);
    }
    else
    {
...
void initializeWeights(float* d_weights, int rows, int cols, const std::string& initType)
{
    ...
    initializeWeightsKernel<<<blocksPerGrid, threadsPerBlock>>>(d_weights, rows, cols, d_states,
                                                                initTypeCode);
    cudaDeviceSynchronize();
...
void MatrixOps::addBias(const float* output, const float* bias, float* result, int batchSize,
                        int outputSize, bool isGPU)
{
    ...
    if (isGPU)
    {
        AddBiasKernel<<<blocksPerGrid, threadsPerBlock>>>(output, bias, result, batchSize,
                                                          outputSize);
    }
    else
    {
        ...
        AddBiasKernel<<<blocksPerGrid, threadsPerBlock>>>(d_output, d_bias, d_result, batchSize,
                                                          outputSize);
...
void MatrixOps::SigmoidGradient(const float* output, float* gradient, int rows, int cols,
                                bool isGPU)
{
    dim3 threadsPerBlock(16, 16);
    dim3 blocksPerGrid((cols + 15) / 16, (rows + 15) / 16);
...
void MatrixOps::elementWiseMultiply(const float* A, const float* B, float* C, int rows, int cols,
                                    bool isGPU)
{
    dim3 threadsPerBlock(16, 16);
    dim3 blocksPerGrid((cols + 15) / 16, (rows + 15) / 16);
