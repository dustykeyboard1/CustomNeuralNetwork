...
void scalerMultiplication(const float* A, float* B, float k, int rows, int cols,
                          bool isGPU = false);
...
void initializeWeights(float* d_weights, int rows, int cols,
                       const std::string& initType = "uniform");
...
void elementWiseMultiply(const float* A, const float* B, float* C, int rows, int cols,
                         bool isGPU = false);
